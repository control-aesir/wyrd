...
use wyrd_daemon::core::{Daemon, LiveError, ResourceBudgets};
...
fn daemon_write_publication_and_retry_converges_across_members() {
    ...
    let (mut live_b, backend_b) = daemon_b.into_live(
        std::time::Duration::from_secs(30),
        ResourceBudgets::default(),
    );
...
fn conflicted_drive_rejects_mounted_writes() {
    ...
    let (live, backend) = daemon.into_live(Duration::from_secs(5), ResourceBudgets::default());
