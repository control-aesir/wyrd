...
/// A sealed rotation fact naming the wrong recipient fails
/// closed: the header correlation must match the obligated pair
/// before the send discharges it. The payload stays sealed (its key
/// is the recipient's), so the header is the whole check.
#[test]
fn delivery_refuses_capability_sealed_bytes_for_another_recipient() {
    ...
    let recipient = identity(0x03).1;
    // A well-formed rotation delivery to the world owner, filed under
    // this obligation's key: only the recipient correlation can catch
    // it — the wrap opens for its own recipient, never for us.
    let state = fx.engine.log.state_of(&child_id).expect("child is valid");
    let owner = *state.members.iter().next().expect("world has a member");
    ...
    .expect("seals")
    .encode();
    // Sanity: the planted bytes are the framing under test.
    assert_eq!(
        SealedRotation::decode(&bytes).expect("decodes").recipient,
        owner
    );
    fx.engine
        .commit_facts(&[
