...
| 27 | Announcement outbox as durable facts: `AnnouncementQueued (snapshot, recipient)` (0x0A, 64 bytes), `AnnouncementSealed (snapshot, sealed bytes)` (0x0B), `AnnouncementDelivered (snapshot, recipient)` (0x0C), plus `AnnouncementRouteSealed (snapshot, route, sealed bytes)` (0x16) for resumes whose live route differs from the first seal's; pending derives as queued-minus-delivered, first seal wins per (snapshot, route) | sender-side retry needs durable per-recipient progress or a partial send strands peers; byte-identical retries collapse in the existing control-message dedupe instead of recording a route-update duplicate per attempt; append-only like every fact (delivered markers accumulate with the dedupe set) |
