...
- **Epoch-secret escrow (T13):** each freshly minted epoch secret is
  ...
  at mint time and persists each record as a keystore sidecar
  (`<store>/escrow/<epoch>`, after the transition commits — a crash
  between them leaves a missing sidecar, never an orphan); vault
