...
- **Epoch-secret escrow (T13):** each freshly minted epoch secret is
  ...
  derived over the record bytes. This is escrow, not derivation — no
   ...
   blob), so the escrowed secrets survive the loss of all other local
   state: the sidecars are the durable bridge to recovery. v0 lifecycle:
   the owner wraps at mint time and persists each record as a keystore
   sidecar (`<store>/escrow/<epoch>`, after the transition commits — a
...
therefore its own framing (envelope version `0x02`, distinct from the
